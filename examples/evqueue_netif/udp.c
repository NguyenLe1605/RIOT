...
extern kernel_pid_t _netif_pid;
...
void *_udp_server(void *args) {
  ...
  while (1) {
    ...
    if ((res = sock_udp_recv(&sock, server_buffer, sizeof(server_buffer) - 1,
...
int udp_send(int argc, char **argv) {
  int res;
  sock_udp_ep_t remote = {.family = AF_INET6, .netif = 9};

  if (argc != 4) {
    puts("Usage: udpse <ipv6-addr> <port> <payload>");
    return -1;
  }
...
int udp_server(int argc, char **argv) {
  ...
  if ((server_running == false) &&
      thread_create(server_stack, sizeof(server_stack),
                    ...
                    _udp_server, argv[1], "UDP Server") <= KERNEL_PID_UNDEF) {
    return -1;
  }
