...
void wireguard_cookie_checker_precompute_peer_keys(
...
static void make_cookie(uint8_t cookie[COOKIE_LEN], sock_udp_ep_t *remote,
  ...
  if (wireguard_expired_birthdate(checker->secret_birthdate,
...
void wireguard_cookie_add_mac_to_packet(void *message, size_t len,
  ...
  if (peer->latest_cookie.valid &&
      !wireguard_expired_birthdate(peer->latest_cookie.birthdate,
                                   COOKIE_SECRET_MAX_AGE -
...
bool wireguard_cookie_message_consume(struct message_cookie_reply *src,
  ...
  if (ret) {
    ...
    peer->latest_cookie.valid = true;
    peer->latest_cookie.have_sent_mac1 = false;
  } else {
    DEBUG("[wireguard] cookie: could not decrypt, invalid cookie response\n");
  }
  return ret != 0;
}
